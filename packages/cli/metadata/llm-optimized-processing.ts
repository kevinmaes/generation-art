...
import type { Individual, Family } from '@generation-art/types';
import type {
  CLIProcessingResult,
  CLIProcessingStats,
  LLMReadyData,
} from '@generation-art/types';
import { stripPIIForLLM } from '@generation-art/utils';
...
export function processGedcomWithLLMOptimization(
  ...
  const llmDataSize = JSON.stringify(llmData).length;
...
export function validateLLMData(
  ...
  Object.entries(llmData.individuals).forEach(([id, individual]) => {
    ...
    if (
      individual.birth &&
      'year' in individual.birth &&
      'date' in individual.birth
    ) {
      issues.push(
    ...
    if (
      individual.death &&
      'year' in individual.death &&
      'date' in individual.death
    ) {
      issues.push(
