...
import { isNumber, isBoolean, isBirthMonth } from '@generation-art/types';
import type {
  Individual,
  Family,
  AugmentedIndividual,
} from '@generation-art/types';
...
export const exampleAddMetadataToExistingPipeline = (
  ...
  console.log(
    `Added metadata to ${String(Object.keys(individualsWithMetadata).length)} individuals`,
  );

  // Example: Check what metadata was added
  const sampleIndividual = Object.values(individualsWithMetadata)[0];
  // if (sampleIndividual) {
  ...
  return Object.values(individualsWithMetadata);
};
