...
function generateCompleteGedcom(): void {
  console.log(
    `Generating complete GEDCOM with ${String(MAX_GENERATIONS)} generations...`,
  );
  ...
  console.log(`Total families to generate: ${String(totalIndividuals - 1)}`);
  ...
  for (let childId = 1; childId <= totalIndividuals; childId++) {
    ...
    if (generation < MAX_GENERATIONS - 1) {
      ...
      const firstParentId =
        Math.pow(2, generation + 1) + (childId - Math.pow(2, generation)) * 2;
