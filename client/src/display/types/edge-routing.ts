...
function canMerge(seg1: EdgeSegment, seg2: EdgeSegment): boolean {
  ...
  if (Math.abs(end1.x - start2.x) > 0.001 || Math.abs(end1.y - start2.y) > 0.001) return false;
...
export function convertLegacyEdge(edge: LegacyEdgeFormat): RoutingOutput {
  const segment = createStraightSegment(edge.source, edge.target, { style: edge.style });
