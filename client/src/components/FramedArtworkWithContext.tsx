...
import { ArtGeneratorWithContext } from './ArtGeneratorWithContext';
...
interface FramedArtworkWithContextProps {
  title: string;
  subtitle?: string;
  width?: number;
  height?: number;
...
export function FramedArtworkWithContext({
  title,
  subtitle,
  ...
  primaryIndividualId,
}: FramedArtworkWithContextProps): React.ReactElement {
  const p5InstanceRef = useRef<p5 | null>(null);
  const [showIndividuals, setShowIndividuals] = useState(true);
  const [showRelations, setShowRelations] = useState(true);

  // Use the context instead of receiving data as props
  const { data: gedcomData } = useFamilyData();
